...
export class QuizComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.apiService.httpClient.get('https://opentdb.com/api.php?amount=10').subscribe((data)=>{
      console.log('data',data);
      this.data = data;
      console.log('DATA',this.data.results)
    });
  }

  nextQuestion() {
    if(this.quizId === this.data.results.length) {
      this.route.navigate(['/result']);
    }
    this.quizId +=1 ;
    console.log('quizid', this.quizId)
    console.log('length', this.data.results.length)
  }
}
